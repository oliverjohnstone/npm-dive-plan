...
function getCNSForChangingDepth(start, end, rate, gas, ppo2) {
  ...
  var startPO2 = ppo2 || gas.getPPO2(fswToMsw(start))
    , endPO2 = ppo2 || gas.getPPO2(fswToMsw(end))
...
function getCNSForFixedDepth(depth, time, gas) {
  var po2 = gas.getPPO2(fswToMsw(depth))
    , tlim
...
function calculateCNS(minPO2, maxPO2, rate, exposureTime, range) {
  ...
  if (mk === 0) {
    return Math.log(Math.abs(tlim * segTime)) - Math.log(Math.abs(tlim))
  } else {
